...
$core_options = new TbCoreControl();

function tb_options($params = array())
{
    ...
    if (is_admin() && !empty($params['id']) && isset($core_options)) {
        ...
        elseif ($pagenow=='post-new.php' || $pagenow=='post.php'){
            ...
            */
            // Render params id
            $params['id'] = "_tb_".$params['id'];
            // Get value
            if(isset($tb_meta->$params['id'])){
                $params['value'] = $tb_meta->$params['id'];
            } else {
                $params['value'] = null;
            }
...
class TbCoreControl
{
        ...
        <div class="images-field clearfix">
            ...
            foreach ($selected as $value):
                ...
                <li class="items" data-id="<?php echo esc_attr($value); ?>" style="background-image:url(<?php echo esc_url($attachment_image[0]);?>);background-size:cover;">
                    <i class="edit dashicons dashicons-plus-alt" title="<?php _e('Replace Image', 'leonard'); ?>"></i>
                    <i class="remove dashicons dashicons-dismiss" title="<?php _e('Remove Image', 'leonard'); ?>"></i>
                </li>
            <?php endif; endforeach; ?>
            <?php if($params['field'] != 'single'): ?>
                <li class="items" data-id=""><i class="add dashicons dashicons-plus-alt" title="<?php _e('Add Image', 'leonard'); ?>"></i></li>
    ...
    private function file($params){
        ...
        <div class="file-field csfield">
    		...
    		<input class="tb_upload_button button button-primary" type="button" value="Browse" />
    		<input data-id="<?php echo esc_attr($params['id']); ?>" class="tb_clear_button button button-danger" type="button" value="Clear" />
        </div>
        <?php
        return ob_get_clean();
    }
    ...
    private function social($params){
		...
		$tb_social_team = json_decode($content);
		wp_enqueue_style('css.ui');
		wp_enqueue_script('jquery.ui.min');
        ob_start();
        ?>
		<table class="wp-list-table widefat fixed pages tb_social_team">
			<thead>
				<tr>
					<th><?php _e('Title', 'leonard');?></th>
					<th><?php _e('Link', 'leonard');?></th>
					<th><?php _e('Icon class', 'leonard');?></th>
					<th><?php _e('Actions', 'leonard');?></th>
				</tr>
			</thead>
			<tfoot>
				<tr>
				...
				<tr>
					<td colspan="3">
						<a class="button-primary" id="btn_add_social" href="javascript:void(0);"><?php _e('Add new', 'leonard');?></a>
					</td>
				</tr>
			</tfoot>
			<tbody  id="the-list">
				<?php
					if(count($tb_social_team)):
						foreach($tb_social_team as $k=>$v): ?>
						<tr>
							...
							<td><a title="Click to edit this item" href="javascript:void(0)" class="tb_social"><?php _e('Edit', 'leonard')?></a>|<a class="tb_social_remove" href="javascript:void(0)"><?php _e('Remove', 'leonard')?></a></td>
						</tr>
						<?php endforeach;
					else: ?>
						<tr>
							<td class="tb_no_item" colspan="3"><?php _e('No item!', 'leonard');?></td>
						</tr>
					<?php endif;
				?>
			</tbody>
        ...
        <div class="social-field csfield">
            <input type="hidden" name="_tb_socials" id="_tb_socials" class="xvalue" value="<?php echo esc_attr($content); ?>"/>
        </div>
        <?php
        return ob_get_clean();
    }
    ...
    public function metabox($params){
        ob_start();
        ?>
		<div id="tb_metabox_field_<?php echo esc_attr($params['id']); ?>" class="tb_metabox_field clearfix">
    ...
    public function save_meta_boxes($post_id)
	{
		...
		/* update _tb_meta_data. */
        if(!empty($tb_meta)){
		    update_post_meta($post_id, '_tb_meta_data', str_replace("\\\'","\\'",json_encode($tb_meta)));
		}
	}
}