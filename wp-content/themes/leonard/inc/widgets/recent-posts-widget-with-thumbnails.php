<?php
class Tb_Recent_Posts_Widget_With_Thumbnails extends WP_Widget {
	...
	function __construct() {
		$this->plugin_slug  = 'tb-recent-posts-widget-with-thumbnails';
        parent::__construct(
            $this->plugin_slug, // Base ID
            __('Recent Posts With Thumbnails', 'leonard'), // Name
            array('description' => __('List of your site&#8217;s most recent posts, with clickable title and thumbnails.', 'leonard'),) // Args
        );
	...
	function widget( $args, $instance ) {
		...
		<?php while ( $r->have_posts() ) : $r->the_post(); ?>
			<li>
				...
				<?php if ( $show_date ) : ?> 
					<div class="tb-post-date"><span><?php echo get_the_date('d F Y'); ?></span></div>
				<?php	endif; ?>
				...
				<!-- Btn read mor -->
				<a href="<?php the_permalink(); ?>" class="btn btn-green btn-readmore-2"><?php _e('Read More', 'leonard');?></a>
				</li>
		<?php endwhile; ?>
		</ul>
...
<?php
	...
	function form( $instance ) {
		...
		?>
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Title:', 'leonard' ); ?></label>
		<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>

		<p><label for="<?php echo esc_attr($this->get_field_id( 'number_posts' )); ?>"><?php _e( 'Number of posts to show:', 'leonard' ); ?></label>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>"><?php _e( 'Display post date?', 'leonard' ); ?></label></p>

		<h4><?php _e( 'Thumbnail Options', 'leonard' ); ?>:</h4>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'show_thumb' )); ?>"><?php _e( 'Display post featured image?', 'leonard' ); ?></label></p>
		
		<p><label for="<?php echo esc_attr($this->get_field_id( 'thumb_width' )); ?>"><?php _e( 'Width of thumbnail', 'leonard' ); ?>:</label>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id( 'thumb_height' )); ?>"><?php _e( 'Height of thumbnail', 'leonard' ); ?>:</label>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'keep_aspect_ratio' )); ?>"><?php _e( 'Use aspect ratios of original images?', 'leonard' ); ?> <em><?php _e( 'If checked the given width is used to determine the height of the thumbnail automatically. This option also supports responsive web design.', 'leonard' ); ?></em></label></p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'hide_title' )); ?>"><?php _e( 'Do not show title?', 'leonard' ); ?> <em><?php _e( 'Make sure you set a default thumbnail for posts without a thumbnail, otherwise there will be no link.', 'leonard' ); ?></em></label> </p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'try_1st_img' )); ?>"><?php _e( "Try to use the post's first image if post has no featured image?", 'leonard' ); ?></label></p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'only_1st_img' )); ?>"><?php _e( 'Use first image only, ignore featured image?', 'leonard' ); ?></label></p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'use_default' )); ?>"><?php _e( 'Use default thumbnail if no image could be determined?', 'leonard' ); ?></label></p>
		
		<p><label for="<?php echo esc_attr($this->get_field_id( 'default_url' )); ?>"><?php _e( 'URL of default thumbnail (start with http://)', 'leonard' ); ?>:</label>
 ...
 * @since 1.0
 */
function tb_register_recent_posts_widget_with_thumbnails () {
	register_widget('Tb_Recent_Posts_Widget_With_Thumbnails');
}
add_action('widgets_init', 'tb_register_recent_posts_widget_with_thumbnails');