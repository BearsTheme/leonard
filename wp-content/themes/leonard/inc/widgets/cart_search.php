...
class WC_Widget_Cart_Search extends WP_Widget {
    public function __construct() {
        parent::__construct(
            ...
            __('Cart & Search', 'leonard'), // Name
            array('description' => __("Display the user's Cart and Search form in the sidebar.", 'leonard'),) // Args
        );
    }
            ...
            <div class="widget_cart_search_wrap_item shopping_cart_dropdown_wrap">						
                <?php if($woocommerce && $show_cart):?>
					...
					<div class="tb_cart_content">
						<span class="cart_total"><?php echo do_shortcode($woocommerce?' '.$woocommerce->cart->get_cart_contents_count():''); _e(' item(s):','leonard');?></span><span class="cart_subtotal"><?php echo do_shortcode($woocommerce->cart->get_cart_subtotal()); ?></span>
					</div>
                <?php endif; ?>
                        ...
                        <div class="shopping_cart_dropdown_inner">
                                    ...
                                    <?php foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) :
                                        ...
                                        <li class="cart-list clearfix">
                                            ...
                                            <div class="cart-list-info">
                                                ...
                                                <?php echo apply_filters( 'woocommerce_widget_cart_item_quantity', '<span class="quantity">' . sprintf( '%s x  %s', $cart_item['quantity'], $remove_link) . '</span>', $cart_item, $cart_item_key ); ?>
                            ...
                            <?php if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) : ?>
                                <div class="cart-total">
                                    <span class="total"><?php _e( 'Total', 'woocommerce' ); ?>:<span><?php echo do_shortcode($woocommerce->cart->get_cart_subtotal()); ?></span></span>
                                    <a href="<?php echo esc_url($woocommerce->cart->get_cart_url()); ?>" class="btn btn-cart"><?php _e( 'View Cart', 'leonard'); ?></a>
									<a href="<?php echo esc_url($woocommerce->cart->get_checkout_url()); ?>" class="btn btn-primary btn-checkout"><?php _e( 'Checkout', 'leonard'); ?></a>
                                </div>
                            <?php endif; ?>
                        </div>
    ...
    function form( $instance ) {
		...
		<p>
            <label for="<?php echo esc_url($this->get_field_id('show_cart')); ?>"><?php _e( 'Show Cart:', 'leonard' ); ?></label>
			<select id="<?php echo esc_attr($this->get_field_id('show_cart')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('show_cart')); ?>">
				<option value="0" <?php selected($show_cart,0); ?>><?php echo __('No','leonard'); ?></option>
				<option value="1" <?php selected($show_cart,1); ?>><?php echo __('Yes','leonard'); ?></option>
			</select>
        </p>
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('show_search')); ?>"><?php _e( 'Show Search:', 'leonard' ); ?></label>
			<select id="<?php echo esc_attr($this->get_field_id('show_search')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('show_search')); ?>">
				<option value="0" <?php selected($show_search,0); ?>><?php echo __('No','leonard'); ?></option>
				<option value="1" <?php selected($show_search,1); ?>><?php echo __('Yes','leonard'); ?></option>
			</select>
        </p>
    <?php
...
if(!function_exists('woocommerce_header_add_to_cart_fragment')){
    function woocommerce_header_add_to_cart_fragment( $fragments ) {
        ...
        ob_start();
        ?>
        <span class="cart_total"><?php echo do_shortcode($woocommerce?' '.$woocommerce->cart->get_cart_contents_count():''); _e(' item(s):','leonard');?></span>
        <?php
