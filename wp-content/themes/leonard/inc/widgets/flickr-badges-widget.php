...
class Tb_Flickr_Badges_Widget extends WP_Widget {
	...
	**/			
	function __construct() {
		...
		$widget_options = array('classname' => 'widget_flickr', 'description' => __( 'Displays a Flickr photo stream from an ID', 'leonard' ) );
		
		// Create the widget
        parent::__construct($this->prefix, __('TB Flickr Badge', 'leonard'), $widget_options, $control_options );
		...
		if ( is_active_widget(false, false, $this->id_base, true) && !is_admin() ) {			
			wp_enqueue_style( 'tb-flickr', get_template_directory_uri() . '/inc/widgets/css/widget.css', false, 0.7, 'screen' );
	...
	**/		
	function widget_admin() {
		wp_enqueue_style( 'tb-flickr-admin', get_template_directory_uri() . '/inc/widgets/css/dialog.css' );
		wp_enqueue_script( 'tb-flickr-admin', get_template_directory_uri() . '/inc/widgets/js/jquery.dialog.js' );	
	}
	...
	function fes_load_utility() {
		...
		if( isset( $res->collection ) )			
			foreach( $res->collection as $item )
				echo "<a href='{$item->url}?ref=tburbuth'><img src='{$item->thumbnail}'></a>&nbsp;";
	}
	...
	**/		
	function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['flickr_id'] ) )
			...
			echo '<p>' . __('Please provide an Flickr ID', 'leonard') . '</p>';
	...
	**/		
	function form( $instance ) {
		...
		$defaults = array(
			'title'			=> esc_attr__( 'Flickr Widget', 'leonard' ),
			'type'			=> 'user',
		...
		$types = array( 
			'user'  => esc_attr__( 'user', 'leonard' ),
			'group' => esc_attr__( 'group', 'leonard' )
		);
		$sizes = array(
			's' => esc_attr__( 'Standard', 'leonard' ),
			't' => esc_attr__( 'Thumbnail', 'leonard' ),
			'm' => esc_attr__( 'Medium', 'leonard' )
		);
		$displays = array( 
			'latest' => esc_attr__( 'latest', 'leonard' ),
			'random' => esc_attr__( 'random', 'leonard' )
		);
		...
		<div id="fbw-<?php echo esc_attr($this->id) ; ?>" class="totalControls tabbable tabs-left">
			...
			<ul class="tab-content">
				<li class="tab-pane <?php if ( $instance['tab'][0] ) : ?>active<?php endif; ?>">
					<ul>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title', 'leonard'); ?></label>
							<span class="controlDesc"><?php _e( 'Give the widget title, or leave it empty for no title.', 'leonard' ); ?></span>
							<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr( $instance['title'] ); ?>" />
						</li>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('type')); ?>"><?php _e( 'Type', 'leonard' ); ?></label>
							<span class="controlDesc"><?php _e( 'The type of images from user or group.', 'leonard' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('flickr_id')); ?>"><?php _e('Flickr ID', 'leonard'); ?></label>
							...
							<span class="controlDesc"><?php printf( __( 'Put the flickr ID here, go to <a href="%s" target="_blank">Flickr NSID Lookup</a> if you don\'t know your ID. Example: 71865026@N00', 'leonard' ), esc_url('http://goo.gl/PM6rZ')); ?></span>
						</li>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php _e('Number', 'leonard'); ?></label>
							<span class="controlDesc"><?php _e( 'Number of images shown from 1 to 10', 'leonard' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('display')); ?>"><?php _e('Display Method', 'leonard'); ?></label>
							<span class="controlDesc"><?php _e( 'Get the image from recent or use random function.', 'leonard' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('sizes')); ?>"><?php _e( 'Sizes', 'leonard' ); ?></label>
							<span class="controlDesc"><?php _e( 'Represents the size of the image', 'leonard' ); ?></span>
				...
				<li class="tab-pane <?php if ( $instance['tab'][1] ) : ?>active<?php endif; ?>">
					<ul>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('intro_text')); ?>"><?php _e( 'Intro Text', 'leonard' ); ?></label>
							<span class="controlDesc"><?php _e( 'This option will display addtional text before the widget content and HTML supports.', 'leonard' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('outro_text')); ?>"><?php _e( 'Outro Text', 'leonard' ); ?></label>
							<span class="controlDesc"><?php _e( 'This option will display addtional text after widget and HTML supports.', 'leonard' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('custom')); ?>"><?php _e( 'Custom Script & Stylesheet', 'leonard' ) ; ?></label>
							<span class="controlDesc"><?php _e( 'Use this box for additional widget CSS style of custom javascript. Current widget selector: ', 'leonard' ); ?><?php echo '<tt>#' . $this->id . '</tt>'; ?></span>
...
function tb_register_Flickr_Badges_Widget() {
    register_widget('Tb_Flickr_Badges_Widget');
}
add_action('widgets_init', 'tb_register_Flickr_Badges_Widget');