...
if( $tb_carousel_lib === 'tb' ) {
    wp_enqueue_script('tb-slick-js', get_template_directory_uri(). '/assets/js/slick.min.js', array('jquery'), '1.0');
    wp_enqueue_style('tb-slick-css', get_template_directory_uri(). '/assets/css/slick.css','','1.0','all');	
    wp_enqueue_script('tb-images-carousel', get_template_directory_uri(). '/assets/js/tb_images_carousel.js', array(), '1.0');
}
...
<?php echo wpb_widget_title( array( 'title' => $title, 'extraclass' => 'wpb_gallery_heading' ) ) ?>
    ...
    <?php if( $tb_carousel_lib !== 'tb' ) : ?>
                            ...
                            <div class="vc_item">
                                <div class="vc_inner">
                                    <?php if ( 'link_image' === $onclick ): ?>
                                        ...
                                        <a class="prettyphoto"
                                           href="<?php echo esc_url($p_img_large[0]) ?>" <?php //echo esc_attr($pretty_rand); ?>>
        ...
        <?php else : ?>
        <div id="<?php echo esc_attr($carousel_id); ?>" class="tb_images_carousel-wrap">
            <div data-slick = '{
                ...
                "infinite" : <?php echo 'yes' === $wrap ? 'true' : 'false' ?>
                 <?php if( 'yes' === $tb_page_image ) echo ',"asNavFor": ".'.$carousel_id.'-nav"'; ?>
                 }'
                 class="tb_images_carousel tb_images_carousel_for <?php echo esc_attr($carousel_id); ?>-for">
                <?php foreach ( $images as $attach_id ): ?>
                    <?php
                    $i ++;
                    if ( $attach_id > 0 ) {
            ...
            <?php if( $tb_page_image === 'yes' ) :?>
            <div class="tb_images_carousel tb_images_carousel_nav <?php echo esc_attr($carousel_id); ?>-nav" data-slick='{
                "asNavFor": ".<?php echo esc_attr($carousel_id); ?>-for",
                "slidesToShow": <?php echo esc_attr($tb_page_image_count); ?>,
                "slidesToScroll": 1,
                "centerMode": true,
                "focusOnSelect": true}'>
                <?php foreach ( $images as $attach_id ): ?>
