...
foreach ( $graph_lines_data as $line ) {
	$unit = ( $units != '' ) ? ' <span class="vc_label_units">' . $units . '</span>' : '';
    ...
    $output .= '<div class="vc_label">' . $line['label'] . '<span class="vc_percent"><strong class="tb-process-bar-counter">' . $percentage_value . '</strong>'.$unit.'</span></div>';
    $output .= '<div class="vc_bar_wrap"><span class="vc_bar' . $bar_options . '" data-percentage-value="' . ( $percentage_value ) . '" data-value="' . $line['value'] . '"' . $line['bgcolor'] . '></span></div>';
	$output .= '</div>';
}

$output .= '</div>';

echo do_shortcode($output . $this->endBlockComment( 'progress_bar' )) . "\n";