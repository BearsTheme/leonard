		...
		<div class="variations">
			<?php $loop = 0; foreach ( $attributes as $name => $options ) : $loop++; ?>
				<select id="<?php echo esc_attr( sanitize_title( $name ) ); ?>" name="attribute_<?php echo sanitize_title( $name ); ?>" data-attribute_name="attribute_<?php echo sanitize_title( $name ); ?>">
					<option value="">
						<?php
							if($name == 'pa_color') echo __( 'Select Color', 'woocommerce' );
							if($name == 'pa_size') echo __( 'Select Size', 'woocommerce' )
						?>
					</option>
					<?php
						if ( is_array( $options ) ) {
				...
				<?php
					if ( sizeof( $attributes ) === $loop ) {
						echo '<a class="reset_variations" href="#reset"><i class="fa fa-close"></i></a>';
					}
				?>
			<?php endforeach;?>
		</div>
