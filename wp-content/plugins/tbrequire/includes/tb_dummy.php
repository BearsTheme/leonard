<?php
$sample_image_id = '';
if(!function_exists('tbDummyData')){
...
if(!function_exists('tbImportThemeOptions')){
	function tbImportThemeOptions($options){
		$options = json_decode($options,true);
		update_option('smof_data',$options);
	}
}
/* Import rev, widget and ess grid*/
if(!function_exists('tbImportRevSlider')){
...
if(!function_exists('tbImportEssGrid')){
	function tbImportEssGrid($theme){
		global $wp_filesystem;
		if (empty($wp_filesystem)) {
		    require_once (ABSPATH . '/wp-admin/includes/file.php');
		    WP_Filesystem();
...
if(!function_exists('tbImportWidgetSetting')){
	function tbImportWidgetSetting($json_data){
		...
		foreach ( $sidebars_data as $import_sidebar => $import_widgets ) :
			foreach ( $import_widgets as $import_widget ) :
				...
				if ( isset( $wp_registered_sidebars[$import_sidebar] ) ) :
					...
					$new_widget_name = tbNewWidgetName( $title, $index );
...
if(!function_exists('tbNewWidgetName')){
	function tbNewWidgetName($widget_name, $widget_index){
