...
jQuery(document).ready(function ($) {
    ...
    $('.tb-masonry').each(function(){
        var $grid = $(this),
            ...
            $filter = $grid.parent().find('.tb-masonry-filter'),
            options = tbMasonry[$parent];
        $grid.find('.tb-masonry-item').append('<div class="shuffle__sizer"></div>');
        options.columnWidth = tb_masonry_col_width($grid, options);

        $grid.imagesLoaded(function(){
            $grid.shuffle({
                itemSelector:'.tb-masonry-item',
                sizer: $grid.find('.shuffle__sizer')
            });
        });
        $(window).resize(function(){
            if($grid.data('resize') == true){
                tb_masonry_col_width($grid, options);
                $grid.shuffle('update');
            }
        });
        if($filter){
            $filter.find('a').click(function(e){
                ...
                // reshuffle grid
                $(this).parent().parent().parent().parent().find('.tb-masonry').shuffle('shuffle', groupName );
            });
        }
    });
    ...
    function tb_masonry_col_width($container, options){
        ...
        $container.find('.tb-masonry-item').each(function() {
            var $item = $(this),
